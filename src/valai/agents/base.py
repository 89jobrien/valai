...
# @lru_cache
# def get_enabled_agents() -> List[str]:
#     """Reads agent toggles from settings and returns a list of enabled agent names."""
#     settings = get_settings()
#     toggler = settings.agent_toggler.model_dump()
#     enabled_agents = [
#         name.replace("use_", "").replace("_agent", "Agent").title()
#         for name, is_enabled in toggler.items()
#         if is_enabled
#     ]
#     console.print(f"Enabled agents: {enabled_agents}")
#     return enabled_agents
...
# @lru_cache
# def load_agents() -> Dict[str, Agent]:
#     """Discovers and loads configurations for all enabled agents.
#     The system prompts for these agents are NOT modified with a timestamp.
#     """
#     agents = {}
#     enabled_agent_names = get_enabled_agents()
#     agent_configs = {
#         Path(config_path).stem.replace("_agent", "Agent").title(): config_path
#         for config_path in glob.glob("config/agents/*.yaml")
#     }
...
#         # Use the clean system prompt directly from the YAML file.
#         prompt = config["system_prompt"]
...
# @lru_cache
# def load_router() -> Agent:
#     """Loads the main router agent. Its system prompt IS injected with the
#     current date and time to provide temporal context for routing decisions.
#     """
#     with open("config/router.yaml", "r") as f:
#         config = yaml.safe_load(f)
#         base_prompt = config["system_prompt"]
...
#     # Inject the dynamic list and timestamp into the router's prompt.
#     full_prompt = (
#         f"{base_prompt}\n\n"
#         f"The current date and time is: {timestamp}.\n"
#         f"Here are the ONLY available specialists you can route to:\n{available_specialists}"
